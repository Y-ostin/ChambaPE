...
exports.handler = async (event) => {
  ...
  try {
    ...
    const reniecResponse = await validateWithRENIEC(
      dni,
      firstName,
      lastName,
      birthDate,
    ...
    if (!reniecResponse.valid) {
      result.rejectionReason =
        reniecResponse.error || 'Datos no coinciden con RENIEC';
    }
...
async function validateWithRENIEC(dni, firstName, lastName, birthDate, apiKey) {
  try {
    const response = await axios.post(
      process.env.RENIEC_API_URL + '/validate',
      {
        dni,
        firstName,
        lastName,
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!sunatResponse.valid) {
      result.rejectionReason =
        sunatResponse.error || 'Problemas en registros SUNAT';
    }
  ...
  try {
    ...
    if (ruc) {
      const rucResponse = await axios.get(
        `${process.env.SUNAT_API_URL}/ruc/${ruc}`,
        {
          headers: {
            Authorization: `Bearer ${apiKey}`,
          },
    ...
    const tributaryResponse = await axios.get(
      `${process.env.SUNAT_API_URL}/tributary/${dni}`,
      {
        headers: {
          Authorization: `Bearer ${apiKey}`,
        },
    ...
    return {
      ...
      error: hasDebt
        ? 'Tiene deudas tributarias pendientes'
        : hasInfractions
          ? 'Tiene infracciones tributarias'
          : !rucActive
            ? 'RUC no está activo'
            : null,
    };
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const certificateBuffer = await getS3Object(
    ...
    const documentAnalysis =
      await analyzeDocumentWithTextract(certificateBuffer);
    ...
    if (!validationResult.valid) {
      result.rejectionReason =
        validationResult.error || 'Problemas en certificado laboral';
    }
    ...
    return {
      ...
      rejectionReason:
        error.message || 'Error interno en validación de antecedentes',
      timestamp: new Date().toISOString(),
    };
  }
};
...
async function analyzeDocumentWithTextract(documentBuffer) {
  try {
    ...
    const extractedText = result.Blocks.filter(
      (block) => block.BlockType === 'LINE',
    )
      .map((block) => block.Text)
      .join(' ');

    const extractedFields = result.Blocks.filter(
    ...
    return {
      ...
      confidence:
        result.Blocks.reduce((acc, block) => acc + block.Confidence, 0) /
...
async function validateCertificateContent(analysis) {
  ...
  return {
    ...
    error: !hasRequiredKeywords
      ? 'Documento no es un certificado laboral válido'
      : !goodConfidence
        ? `Calidad del documento muy baja (${confidence.toFixed(1)}%)`
        : hasProblems
          ? 'Se encontraron antecedentes problemáticos'
          : null,
  };
}
```
...
export class WorkersController {
  ...
  async registerWorkerWithValidation(
    ...
    const worker =
      await this.workersService.createPendingWorker(createWorkerDto);
    ...
    return {
      message:
        'Registro iniciado. Te notificaremos el resultado de la validación.',
      workerId: worker.id,
      validationId,
